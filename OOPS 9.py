...
1. Public Access Specifiers are the variables of a class which can be accessed by any other class be it a subclass of the class or
not. If we simply name a variable in python it is is declared public by default

2. Protected  Access Specifiers are the variables of a class which can be accessed only by the base class ot the sub classes of the
base class.To declare a variable as protected we type a 'single underscore' before our variable name.For Example:-
'_xyz' is an example of a protected variable.There should not be ay space between underscore and variable name.

3. Private  Access Specifiers are the variables which can be accessed only by their own class i.e. the class in which they are decla
-red .To declare a variable as Private we type two underscores before our variable name.for example:-
...
print(slash._user) #user can be called by objext slash of subclass Musician because user is a Protected variabe & can be acced by
# class Fender and all of its subclasses(Musician being one such subclass)
